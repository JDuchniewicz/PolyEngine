...
String pe::api::resources::EvaluateFullResourcePath(eResourceSource Source, const String& path)
{
	return gAssetsPathConfig.GetAssetsPath(Source) + path;
}

String pe::api::resources::LoadTextFileRelative(eResourceSource Source, const String & path)
{
...
void pe::api::resources::SaveTextFileRelative(eResourceSource Source, const String& path, const String& data)
{
	String absolutePath = EvaluateFullResourcePath(Source, path);
	SaveTextFile(absolutePath, data);
}

float* pe::api::resources::LoadImageHDR(const String& path, int* width, int* height, int* fileChannels)
{
	if (!stbi_is_hdr(path.GetCStr()))
	{
		gConsole.LogWarning("pe::api::resources::LoadImageHDR LDR file spotted, scalling to HDR and gamma change may appear!");
	}
	...
	if (!data)
	{
		gConsole.LogInfo("pe::api::resources::LoadImageHDR Failed to load: {}, reason: {}", path, stbi_failure_reason());
	}
	
	return data;
}

void pe::api::resources::FreeImageHDR(float* data) 
{
	stbi_image_free(data);
}

unsigned char* pe::api::resources::LoadImage(const String& path, int* width, int* height, int* fileChannels, int desiredChannels)
{
	if (stbi_is_hdr(path.GetCStr()))
	{
		gConsole.LogWarning("pe::api::resources::LoadImageHDR HDR file spotted, scalling to LDR and gamma change may appear!");
	}
	...
	if (!data)
	{
	 gConsole.LogInfo("pe::api::resources::LoadImage Failed to load: {}, reason: {}", path, stbi_failure_reason());
	}
	
	return data;
}

void pe::api::resources::FreeImage(unsigned char* data)
{
	stbi_image_free(data);
}
